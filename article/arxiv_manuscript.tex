...
\hypersetup{
  ...
  pdftitle={Rare-Event Recurrence Diagnostics for Econometric Stress Analysis}
}
...
\newcommand{\figplaceholder}[2]{%
  \begin{figure}[H]
    ...
    \IfFileExists{#1}{%
      ...
      \fbox{%
        \parbox[c][0.28\textheight][c]{0.86\textwidth}{%
          \centering
          Figure asset not bundled in this working tree.\\
          Expected file: \texttt{#1}%
        }%
      }%
    }
    \caption{#2}
...
\title{Rare-Event Recurrence Diagnostics for Econometric Stress Analysis}
\author{Author Name\\
Institution\\
...
\begin{enumerate}[leftmargin=1.5em]
  \item a reproducible synthetic comparison showing that large events can differ in recurrence and clustering structure even when they look superficially similar;
  \item a compact diagnostic workflow for exceedances, return times, extremal-index summaries, and multivariate stress timing;
  \item an empirical application template with explicit data-loading, catalog, and cache-refresh paths that is ready for disciplined use on real public datasets.
\end{enumerate}
...
\section{Why Recurrence Deserves Separate Attention}
...
\section{Conceptual Translation}
...
\toprule
\textbf{Dynamical-systems idea} & \textbf{Econometric interpretation} \\
\midrule
...
\bottomrule
\end{tabular}
\caption{Conceptual translation from recurrence language to econometric diagnostics.}
\end{table}
...
\begin{itemize}[leftmargin=1.5em]
  \item threshold exceedance extraction;
  \item runs-based extremal-index summaries;
  \item return-time and recurrence diagnostics;
  \item multivariate joint-stress summaries;
  \item descriptive comparison with baseline volatility-style diagnostics;
  \item saved article-facing figures with matching source CSV files.
\end{itemize}
...
The stable article-facing assets are:

\begin{itemize}[leftmargin=1.5em]
  \item \texttt{01\_conceptual\_pipeline.png} for the workflow from raw inputs to article evidence;
  \item \texttt{02\_synthetic\_extremes\_comparison.png} for clustered versus more isolated synthetic extremes;
  \item \texttt{03\_return\_time\_distribution.png} for descriptive return-time behavior;
  \item \texttt{04\_extremal\_index\_by\_series.png} for cross-series clustering comparison;
  \item \texttt{05\_multivariate\_stress\_timeline.png} for joint-stress timing;
  \item \texttt{06\_econometric\_vs\_recurrence\_diagnostics.png} for descriptive comparison between baseline volatility-style summaries and recurrence-oriented diagnostics.
\end{itemize}
...
\section{Computational Workflow and Reproducibility}
...
\begin{enumerate}[leftmargin=1.5em]
  \item load or simulate a panel in the repository's standard long format;
  \item define a threshold-based stress region;
  \item extract exceedances and recurrence summaries;
  \item compute clustering diagnostics such as the runs-based extremal index;
  \item save article-facing outputs with matching CSV sources.
\end{enumerate}
...
\figplaceholder{figures/01_conceptual_pipeline.png}{Conceptual pipeline from input panel to article-facing evidence. This figure summarizes the computational workflow used in the repository: data loading or simulation, threshold definition, exceedance extraction, recurrence and clustering diagnostics, and saved article-facing outputs. It illustrates the analysis structure rather than an empirical result.}

\section{Synthetic Evidence as the Current Strongest Support}
...
In article terms, that synthetic support is most directly expressed through \texttt{02\_synthetic\_extremes\_comparison.png}, which is the clearest visual anchor for the claim that tail behavior differs not only in magnitude but also in recurrence structure.
...
\figplaceholder{figures/02_synthetic_extremes_comparison.png}{Synthetic comparison of clustered and more isolated extremes. This figure compares benchmark processes with different recurrence structure. It supports the methodological claim that similar-looking large observations can differ materially in clustering behavior. It does not constitute empirical evidence about a specific economy.}

\section{Extremal Index as a Practical Diagnostic}
...
The most direct article-facing support for this point is \texttt{04\_extremal\_index\_by\_series.png}, which summarizes cross-series clustering differences without requiring a stronger structural interpretation than the code currently supports.
...
\figplaceholder{figures/04_extremal_index_by_series.png}{Extremal-index comparison across series or benchmark observables. This figure reports cross-series differences in clustering diagnostics. Lower values indicate more clustered extreme episodes in the sample or benchmark workflow. The figure supports comparative interpretation, not universal threshold rules.}

\section{Return Times and Recurrence Summaries}
...
That descriptive role is exactly what \texttt{03\_return\_time\_distribution.png} supports. It is evidence about recurrence timing patterns, not a device for exact crisis-date prediction.
...
\figplaceholder{figures/03_return_time_distribution.png}{Return-time distribution for a benchmark observable. This figure summarizes descriptive return-time behavior for repeated visits to a threshold-defined stress region. It should be read as a recurrence diagnostic, not as a forecast of the next crisis date.}

\section{Multivariate Stress and Joint Recurrence}
...
The relevant stable visual here is \texttt{05\_multivariate\_stress\_timeline.png}, which supports a joint-stress timing narrative but not a causal account of why the co-movement occurs.
...
\figplaceholder{figures/05_multivariate_stress_timeline.png}{Multivariate stress timeline. This figure shows the timing of joint stress across observables in a multivariate setting. It supports descriptive claims about co-occurrence and joint recurrence, but it does not identify a causal transmission mechanism.}

\section{Data and Empirical Application Template}
...
\section{Empirical Use: Template, Not Result}
...
\section{Relationship to Standard Econometric Baselines}
...
That complementarity is what \texttt{06\_econometric\_vs\_recurrence\_diagnostics.png} is for. The figure is strongest when used to compare descriptive lenses, not when used to claim that one framework fully dominates the other.
...
\figplaceholder{figures/06_econometric_vs_recurrence_diagnostics.png}{Baseline econometric versus recurrence-oriented diagnostics. This figure compares volatility-style baseline summaries with recurrence-oriented rare-event diagnostics. It supports the claim that the two views are complementary rather than interchangeable.}

\section{How the Figures Should Be Used in the Article}

The article is strongest when each major claim is attached to one of the stable numbered figures and its source CSV:

\begin{itemize}[leftmargin=1.5em]
  \item Figure 1 supports the computational workflow description.
  \item Figure 2 supports the central methodological motivation.
  \item Figure 3 supports descriptive recurrence timing statements.
  \item Figure 4 supports clustering comparisons.
  \item Figure 5 supports descriptive joint-stress timing.
  \item Figure 6 supports complementarity with standard baseline summaries.
\end{itemize}
...
\begin{itemize}[leftmargin=1.5em]
  \item foundational work on extreme value theory and recurrence in dynamical systems;
  \item applied references on extremal index estimation and declustering;
  \item standard econometric references for baseline volatility and persistence diagnostics;
  \item any data-source references used in a concrete empirical application;
  \item repository citation metadata for the computational artifact itself.
\end{itemize}
