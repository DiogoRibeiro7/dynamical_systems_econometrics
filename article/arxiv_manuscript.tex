...
=(1-p(u))^r,
\]
so the relevant finite-threshold benchmark is exactly the probability that the preceding $r$ observations remain below threshold under temporal independence. Substituting this value into the definition of $\Lambda$ gives the finite-level independence benchmark, and the asymptotic statement follows because $p(u_n)\to 0$ implies $(1-p(u_n))^r\to 1$. If $r_2>r_1$, then the event
\[
...
This lemma is modest in intent. It collects the algebraic consequences of the definition and identifies the exact finite-threshold independence benchmark, but it does not claim a deeper structural theorem about clustered extremes.
...
Taken together, the results in this subsection make a limited theoretical point. The first lemma records basic finite-threshold ordering and benchmark properties, and the second transfers consistency from established ingredients to the proposed functional. Their role is to keep the object mathematically legible, not to suggest that the paper resolves the harder theory of extremal-index estimation.
