...
At that baseline specification, the recurrence ranking is clear. Unemployment stress is by far the most concentrated object in the panel: threshold `8.2`, `42` exceedances, only `2` runs clusters, extremal index `0.048`, and cluster-adjusted stress `2.028`. The Baa-Treasury spread is much less concentrated and more episodic across distinct stress windows: threshold `3.16`, `43` exceedances, `8` clusters, extremal index `0.186`, and cluster-adjusted stress `0.531`. KCFSI is similar: threshold `1.068`, `44` exceedances, `8` clusters, extremal index `0.182`, and cluster-adjusted stress `0.556`. The substantive point is that these three indicators have broadly similar tail incidence but very different temporal organization. Labor-market stress arrives in a small number of long episodes; credit and financial stress recur in more distinct bursts.

That ranking survives uncertainty, but not with equal precision across series. Using a `250`-replication moving-block bootstrap with `36`-month blocks, the unemployment estimate is the least stable object in the panel, which is exactly what one should expect when clustering is being inferred from only `2` long episodes. Its interval for `theta` is `[0.048, 0.189]`, and the corresponding interval for `Lambda` is `[0.450, 2.028]`. For the Baa-Treasury spread, the intervals are `[0.119, 0.306]` and `[0.318, 0.811]`; for KCFSI they are `[0.136, 0.330]` and `[0.300, 0.742]`. The ranking therefore survives the uncertainty treatment, but the unemployment estimate is visibly more fragile because the denominator of the cluster-adjusted functional is being learned from very few empirical clusters.

The new empirical robustness figure should carry much of the remaining exposition. Along the threshold dimension, unemployment is the most sensitive series: `Lambda` falls from `2.028` at `q = 0.90` to `0.838`, `0.415`, and `0.130` at `q = 0.93`, `0.95`, and `0.97`. That pattern indicates that the broader upper-tail unemployment regime is dominated by a few long episodes, whereas the very highest realizations are less tightly packed. The Baa-Treasury spread and KCFSI decline more smoothly, moving from `0.531` to `0.130` and from `0.556` to `0.150` over the same grid. Along the run-length dimension, the comparison changes. Unemployment is essentially invariant because its two stress episodes are already separated by long calm intervals, while the two financial indicators become progressively more clustered as the episode definition widens. The Baa-Treasury spread rises from `0.425` at `r = 1` to `0.607` by `r = 4`; KCFSI rises from `0.371` at `r = 1` to `0.742` by `r = 6`. The strongest empirical reading is therefore not that one series dominates under every design choice, but that labor-market stress dominates at moderate high thresholds while credit and financial stress are the more declustering-sensitive objects.
