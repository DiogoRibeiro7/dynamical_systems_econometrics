...
The empirical illustration should no longer be framed as a toy example. The repo now supports a small but real monthly panel built from three FRED series: the civilian unemployment rate, the Moody's Baa minus 10-year Treasury spread, and the Kansas City Financial Stress Index. After alignment on the common sample, the panel runs from February 1990 through May 2026 and contains `435` observations. To keep the comparison with the synthetic section clean, the empirical diagnostics use upper-tail exceedances at the `0.90` quantile and a runs declustering parameter of `r = 3`.

The recurrence differences across the three series are substantive. Unemployment stress is extremely concentrated: threshold `8.2`, `42` exceedances, only `2` runs clusters, extremal index `0.048`, and cluster-adjusted stress `2.028`. The Baa-Treasury spread is less concentrated and more episodic across distinct stress windows: threshold `3.16`, `43` exceedances, `8` clusters, extremal index `0.186`, and cluster-adjusted stress `0.531`. The financial-stress index behaves similarly: threshold `1.068`, `44` exceedances, `8` clusters, extremal index `0.182`, and cluster-adjusted stress `0.556`. The contrast matters because it shows that variables with similar exceedance incidence can still encode very different persistence structures.

The empirical section should now report its own bootstrap uncertainty rather than relying on the synthetic threshold scan as a proxy for credibility. Using a `250`-replication moving-block bootstrap with `36`-month blocks, the unemployment estimate is visibly the least precise object in the panel, which is the natural finite-sample consequence of learning clustering from only `2` long episodes. Its bootstrap interval for `theta` is `[0.048, 0.189]` and the corresponding interval for `Lambda` is `[0.450, 2.028]`. For the Baa-Treasury spread, the intervals are `[0.119, 0.306]` for `theta` and `[0.318, 0.811]` for `Lambda`. For KCFSI, the intervals are `[0.136, 0.330]` and `[0.300, 0.742]`. The economic ranking survives that uncertainty treatment, but the unemployment estimate is plainly more fragile than the other two because the denominator of the cluster-adjusted functional is being estimated from very few clusters.

Threshold sensitivity should now be reported explicitly for the empirical panel as well. The unemployment series changes the most as the cutoff rises: `Lambda` falls from `2.028` at `q = 0.90` to `0.838`, `0.415`, and `0.130` at `q = 0.93`, `0.95`, and `0.97`, while `theta` rises from `0.048` to `0.231`. That pattern says something substantive rather than merely technical. Broad labor-market distress is highly clustered, but the very highest unemployment realizations are less concentrated than the wider upper-tail regime. The Baa-Treasury spread and KCFSI are more stable across the threshold grid. Their `Lambda` paths move from `0.531` to `0.130` and from `0.556` to `0.150`, respectively, which still indicates meaningful threshold sensitivity, but not the same collapse from an exceptionally small initial extremal index. The empirical ranking is therefore strongest at moderate high thresholds and becomes more compressed at more severe cutoffs.

Run-length sensitivity adds a different kind of robustness check because it tests whether the empirical ranking depends on how aggressively nearby exceedances are merged into common episodes. The unemployment series is effectively invariant: at `q = 0.90`, its `Lambda` estimate remains `2.028` from `r = 1` through `r = 8`, which means the two labor-market stress clusters are already separated by much longer quiet intervals than any declustering window considered here. The financial indicators are more design-sensitive. The Baa-Treasury spread rises from `0.425` at `r = 1` to `0.472` at `r = 2` and `0.607` by `r = 4`, then stabilizes. KCFSI rises from `0.371` to `0.556`, `0.636`, and `0.742` as the run length reaches `2`, `4`, and `6`. That is the cleanest evidence so far that the recurrence framework is picking up genuine differences in temporal organization rather than only differences in tail incidence: labor-market stress is already organized into long blocks, whereas credit and financial stress become progressively more clustered as the analyst widens the episode definition.

At this point the empirical section also needs one visual object that collects both robustness dimensions in one place. The threshold path and the run-length path should be shown side by side for the three series. That figure does more than save space. It makes the comparative logic immediate: unemployment dominates at moderate thresholds but compresses quickly as the cutoff rises, while the financial indicators are less threshold-sensitive but more declustering-sensitive.
...
That is the proper role of the section. It is still modest, because there is no structural identification and no claim of optimal thresholding, but it is no longer a placeholder. It demonstrates that the framework can separate long, concentrated macro stress from repeated but more episodic credit and financial stress, and it does so with an artifact that the repo now generates directly.
